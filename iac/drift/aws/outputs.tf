...
output "drift_marker" {
  ...
  #
  # `nonsensitive()` because the AWS provider marks every `aws_ssm_parameter.value` sensitive,
  # regardless of type, and a sensitive root output is a hard error. This parameter is a `String`
  # holding a literal drift marker — the harness writes it with `aws ssm put-parameter --value`
  # and compares it as plain text — so nothing is being unwrapped that was ever a secret. Marking
  # the OUTPUT sensitive instead would also work and is worse: it hides a value whose entire
  # purpose is to be read and compared.
  value = nonsensitive(aws_ssm_parameter.drift_probe.value)
}
